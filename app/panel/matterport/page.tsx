...
import MatterportClientPage, {
  type DbMatterportBooking,
  type DbMatterportTour,
} from "./MatterportClientPage"

export default async function MatterportPage() {
  ...
  const [{ data: bookingRows }, { data: tourRows }] = await Promise.all([
    ...
    admin
      .from("subscriptions")
      .select(
        "id, contact_id, status, ends_at, contacts(full_name, company)"
      )
  ...
  const bookings: DbMatterportBooking[] = (bookingRows ?? []).map((row) => {
    ...
    return {
      ...
      status: row.status,
      matterport_status: ((row as Record<string, unknown>).matterport_status as string ?? "scheduled") as import("./MatterportClientPage").MatterportStatus,
  ...
  return <MatterportClientPage bookings={bookings} tours={tours} />
}
