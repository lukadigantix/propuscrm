...
type TourBooking = {
  ...
  contact: TourContact | null
}

type TourDetailData = {
  booking: TourBooking
  sub: { status: string; ends_at: string } | null
}
...
export default function TourDetailClient({ id }: { id: string }) {
  ...
  const { data, isLoading } = useQuery<TourDetailData | null>({
    queryKey: ["matterport-tour", id],
    queryFn: () => {
      console.log(`[TourDetailClient] fetching /api/matterport/tours/${id}...`)
      return fetch(`/api/matterport/tours/${id}`).then((r) => {
  ...
  async function handlePending() {
    setSaving(true)
    try {
      await setMatterportPending(id)
      queryClient.invalidateQueries({ queryKey: ["matterport-tour", id] })
      queryClient.invalidateQueries({ queryKey: ["matterport"] })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleArchive() {
    setSaving(true)
    try {
      await setMatterportArchived(id)
      queryClient.invalidateQueries({ queryKey: ["matterport-tour", id] })
      queryClient.invalidateQueries({ queryKey: ["matterport"] })
    } finally {
      setSaving(false)
  ...
  async function handleSaveLink(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await saveMatterportLink(formData)
      queryClient.invalidateQueries({ queryKey: ["matterport-tour", id] })
      queryClient.invalidateQueries({ queryKey: ["matterport"] })
    } finally {
      setSaving(false)
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <Box className="size-12 text-muted-foreground/30 mb-4 mx-auto" />
            <p className="text-sm font-medium text-muted-foreground">Tour not found.</p>
          </div>
        </div>
      </div>
    )
  ...
  const { booking, sub } = data
  const shortId       = booking.id.slice(0, 8).toUpperCase()
  const matterportLink = booking.matterport_link
  const mpStatus       = booking.matterport_status
            ...
            <div className="flex items-center gap-2 flex-wrap shrink-0">
              {mpStatus === "scheduled" && (
                <button
                  ...
                  disabled={saving}
                  className="inline-flex items-center gap-1.5 rounded-lg border bg-background px-3 py-1.5 text-sm font-medium hover:bg-muted transition-colors disabled:opacity-50"
                >
              ...
              {matterportLink ? (
                <>
                  ...
                  <div className="px-5 py-4 border-t bg-muted/40">
                    <form onSubmit={handleSaveLink} className="flex items-center gap-2">
                      ...
                      <button
                        type="submit"
                        disabled={saving}
                        className="rounded-lg bg-foreground text-background px-3 py-1.5 text-xs font-medium hover:opacity-80 transition-opacity shrink-0 disabled:opacity-50"
                      >
                        Update
                      </button>
                    </form>
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-16 text-center gap-4 px-6">
                  ...
                  {mpStatus === "pending" && (
                    <form onSubmit={handleSaveLink} className="w-full max-w-sm flex flex-col gap-2">
                      <input type="hidden" name="bookingId" value={id} />
                      <input
                        type="url"
                        name="link"
                      ...
                      <button
                        type="submit"
                        disabled={saving}
                        className="w-full rounded-lg bg-foreground text-background px-4 py-2.5 text-sm font-semibold hover:opacity-80 transition-opacity flex items-center justify-center gap-2 disabled:opacity-50"
                      >
